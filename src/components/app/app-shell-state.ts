...
export function pageTitle(pathname: string): string {
  ...
  if (pathname.startsWith("/lessons")) return "홈";
  if (pathname.startsWith("/guest")) return "홈";
  ...
  if (pathname.startsWith("/club/schedule")) return "출석";
  ...
  if (pathname.startsWith("/club/manage/guest")) return "동호회";
  if (pathname.startsWith("/club/manage")) return "회비";
...
export function selectedNativeTab(pathname: string): NativeTabId | null {
  ...
  if (normalized === "/guest" || normalized.startsWith("/guest/")) return "home";
