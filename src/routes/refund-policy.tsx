...
export const Route = createFileRoute("/refund-policy")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "민턴동 게스트 예약의 취소, 환불 기준과 결제 취소 처리 원칙을 안내합니다.",
      },
      { property: "og:title", content: "취소 및 환불 정책 – 민턴동" },
      { property: "og:description", content: "게스트 예약 취소 및 환불 원칙을 확인하세요." },
    ],
  }),
...
function RefundPolicyPage() {
  const sections: LegalSection[] = [
    {
      ...
      content: (
        <p>
          결제 후 7일 이내이면서 해당 게스트 운동이 시작되기 전이라면 전액 환불을 원칙으로 합니다.
    ...
    },
    {
      ...
      content: (
        <p>
          게스트 예약은 운동 시작 전까지 취소를 요청할 수 있습니다. 결제 후 7일 이내이면서 운동 시작
          전인 경우 전액 환불합니다. 이미 정상적으로 제공이 완료된 예약은 해당 예약에 대한 환불이
          제한될 수 있습니다. 다만 상품 내용이 고지와 다르거나 정상적으로 제공되지 않은 경우에는
          관계 법령에 따른 권리를 보장합니다.
        </p>
      ),
    },
    {
      ...
      content: (
        <p>
          모집 일정이 일부 진행된 뒤 취소되는 경우에는 이미 정상적으로 제공된 이용분을 제외하고 남은
          미이용 부분을 기준으로 환불 금액을 산정합니다. 모집별로 별도의 공제 기준이나 취소 조건이
          있는 경우에는 결제 전에 명확히 표시하고 동의를 받습니다.
        </p>
      ),
    },
    {
      ...
      content: (
        <p>
          동호회·시설 사정 등 이용자에게 책임이 없는 사유로 게스트 운동을 제공할 수 없으면 대체
    ...
    },
    {
      ...
      content: (
        <p>
          예약번호, 게스트 운동 일시와 취소 사유를 준비해 <Link to="/business-info">고객센터</Link>
          로 문의해 주세요. 신청 당시 안내와 다르게 처리되었거나 상품 내용과 다르게 제공된 경우 확인
          후 관계 법령에 따라 조치합니다.
        </p>
      ),
    },
  ...
  return (
    <LegalPage
      title="취소 및 환불 정책"
      description="게스트 예약과 결제 취소가 어떤 원칙으로 처리되는지 안내합니다."
