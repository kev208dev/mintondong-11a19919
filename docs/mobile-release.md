...
Apple 로그인을 사용하는 계정 삭제 시 Apple authorization token 폐기까지 확인해야 한다. 현재
서버 계정 삭제는 Supabase Auth user를 삭제하지만 Apple token 폐기는 개발자 자격증명과 실기기
검증 전이므로 스토어 제출 전 별도 확인 항목이다.
...
`supabase/migrations/20260813035134_account_deletion.sql` 적용 후 동작한다.
...
- Apple Developer: App ID/signing/provisioning, Sign in with Apple association, associated callback,
...
- Production DB: review/apply the account deletion migration
